...
public class ExampleStreams {
    ...
    private static final List<ExampleStreams.Person> PERSONS = new ArrayList<>();

    // loop through the persons in a given list
    public void printPersonList(Collection<Person> persons) {
    ...
    protected  List<Person> findEmployedPersons(List<Person> persons) {
    ...
    protected  List<Person> findAllPersonsByLastName(List<Person> persons, String lastName) {
        return persons.stream()
    ...
    protected Person findFirstPersonByFirstName(List<Person> persons, String firstName) {
        return persons.stream()
    ...
    protected  Set<String> getUniqueFirstNames(List<Person> persons) {
        return persons.stream()
    ...
    protected double calculateAverageSalary(List<Person> persons) {
        return persons.stream()
                .filter(p -> p.isEmployed()) // .filter(Person::isEmployed())
    ...
    public  Map<Boolean, List<Person>> groupByEmploymentStatus(List<Person> persons) {
        return persons.stream()
                .collect(Collectors.groupingBy(Person::isEmployed));
    }

    // 8. TODO: Group persons by first name
    protected  Map<String, List<Person>> groupByFirstName(List<Person> persons) {
        return persons.stream()
